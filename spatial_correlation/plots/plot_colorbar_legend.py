...
def plot_standalone_colorbar_biopsytype(patient_colors, labels, save_folder):
    """Plot a standalone vertical and horizontal Colorbar
    ...
    cbar.set_ticks(np.linspace(0, num_colors, num_colors + 1)[:-1] + 0.5)
    cbar.ax.set_yticklabels(labels, fontsize=xy_ticks, rotation=0, ha="left")
    ax.set_title('biopsy type', fontsize=title_fontsize)
    plt.tight_layout()
    plt.savefig(os.path.join(save_folder, "Biopsytype_colorbar.pdf"))
    plt.close()
