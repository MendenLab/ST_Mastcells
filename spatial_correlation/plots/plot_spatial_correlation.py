...
def plot__stwc_tissuelayers(df_counts, cytokine_responders, save_folder, distance, corr_method):
    ...
    for cyto in cytokine_responders:
        ...
        elif (temp_df[cyto].max() > 1000) & (temp_df[cyto].max() < 2000):
            ax.set_yticks(np.arange(0, temp_df[cyto].max() + increased_ylimit, 400))
        else:
        ...
        if any(get_sizes > 6):
            ...
            ax.text(0, temp_df[cyto].max() + 1,
                    "weighted {}'s r = {:.2f}\np = {:.2e}".format(
                        corr_method.capitalize(), corr_pval[0][1], corr_pval[0][2]),
...
def plot__stwc_biopsytype(df_counts, cytokine_responders, save_folder, distance, corr_method):
    """Calculate Correlation between each Cytokine positive spot and its nn responder genes spots for each sample
    -> Plot for Figure 4G-I
    ...
    # Get unique combinations of the tissue layer to assign to each a color before the plotting
    # 1. assign a color to each combination
    dict_biopsytype_color = {'LESIONAL': 0, 'NON LESIONAL': 1}
    num_unique_colors = len(dict_biopsytype_color.values())
    biopsytypecomb_colors = plt.cm.get_cmap("tab20", num_unique_colors)

    # 2. Plot Colorbar with tissue layer combinations
    plot_colorbar_legend.plot_standalone_colorbar_biopsytype(
        patient_colors=biopsytypecomb_colors, labels=dict_biopsytype_color.keys(), save_folder=save_folder)
    ...
    for cyto in cytokine_responders:
        resp_name = "_".join([cyto, 'responder'])
        temp_df = df_counts[[
            cyto, resp_name, "Cluster_size", "disease", 'tissue_layer', 'biopsy_type', 'Cluster_num_spots']].copy()
        weighted_cytoname = "_".join(['weighted', cyto])
        ...
        # # map each string to an integer value
        cyto_colorseq = []
        for ind_dict, value in enumerate(temp_df['biopsy_type']):
            cyto_colorseq.append(dict_biopsytype_color[value])
        ...
        # Plot unweighted points
        ax.scatter(data=temp_df, x=resp_name, y=cyto, c=cyto_colorseq, s=100, alpha=0.8,
                   cmap=ListedColormap(biopsytypecomb_colors.colors[np.unique(cyto_colorseq)]), zorder=2)
        ...
        # 3. Save figure -> creates figure for Workflow figure 1
        fig.savefig(os.path.join(
            save_folder, "{}_biopsy_type_weighted_transcripts_{}_r{}_{}_{}{}".format(
                'FigS', corr_method, str(distance), cyto, resp_name, fileformat)),)
        plt.close()
...
def plot__stwc_tissuelayers_figure1g(df_counts, cytokine_responders, save_folder, distance, corr_method):
    ...
    for cyto in cytokine_responders:
        ...
        # Axis params
        ax.set_xlabel("CXCL17 transcript level", fontsize=18)
        ax.set_ylabel(" ".join([cyto.split('_')[0], 'transcript level']), fontsize=18)
        ...
        #         temp_df[cyto].max() + 1,
        #         'r = {:.2f}\np = {:.2e}'.format(corr_pval[0][1], corr_pval[0][2]),
        #         fontstyle='italic', fontsize=text_fontsize)
        # Only show correlation value - in my opinion we should report the p-value at least in the text ...
        ax.text(0, temp_df[cyto].max() + 1,
                "weighted {}'s r = {:.2f}\np = {:.2e}".format(
                    corr_method.capitalize(), corr_pval[0][1], corr_pval[0][2]),
        ...
        fig.savefig(os.path.join(
            save_folder, "{}_tissuelayers_weighted_transcripts_{}_r{}_{}_{}{}".format(
                'Fig_2v', corr_method, str(distance), cyto, resp_name, fileformat)),)
        plt.close()
