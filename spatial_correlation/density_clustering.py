...
def get_cluster_counts(adata, tissue_types, cytokine_responders, save_folder, distance=1, find_associated_genes=False,
    ...
    col_names.extend(['disease', 'Cluster_size', 'Cluster_num_spots', 'Specimen', 'biopsy_type'])
    ...
    for ind_specimen, specimen in enumerate(specimen_names):
        ...
        for cyto in cytokine_responders.keys():
            ...
            if len(cyto_coordinates) > 0:
                ...
                subgraph_cyto_spots = cyto_graphs.get_connections(
                    coordinates=np.array(cyto_coordinates), distance=2)
                ...
                for indices_spots in final_cluster_indices:
                    ...
                    for resp_gene in responder_genes:
                        # We always want to read out the raw counts
                    ...
                    df_counts.at[index_counter, 'biopsy_type'] = np.unique(sub_adata.obs['biopsy_type'])[0]
                ...
                if get_plots:
                    ...
                    plot_clusters.plot_clusters_counts(
                        sample_adata=sub_adata, cluster_adata=sevenspots_adata, cyto=cyto, save_folder=save_folder,
                        obs_label=obs_label, obs_counts=obs_counts)
                ...
                df_excluded_spot_counts = pd.concat([df_excluded_spot_counts, df_temp], ignore_index=True)
                ...
                df_temp = pd.DataFrame(included_adata_obs[obs_resp_counts][m_resp].values,
                                       columns=["_".join([cyto, 'responder'])])
                df_included_spot_counts = pd.concat([df_included_spot_counts, df_temp], ignore_index=True)
            else:
...
def run_spatialcorrelation(adata, tissue_types, cytokine_responders, save_folder, radius, sig, corr_method,
    ...
    _ = plot_spatial_correlation.plot__stwc_biopsytype(
        df_counts=df_counts, cytokine_responders=cytokine_responders, save_folder=save_folder,
        distance=radius, corr_method=corr_method)
